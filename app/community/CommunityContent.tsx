...
export default function CommunityContent() {
    ...
    const resources = [
        ...
        { icon: <HelpCircle className="w-6 h-6" />, title: "Help Center", desc: "FAQs & troubleshooting", link: "/faqs" },
    ...
    const upcomingEvents = [
        { title: "Weekly Office Hours", date: "Every Thursday 11 AM IST", type: "Live Q&A" },
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                        {communities.map((comm, i) => (
                            <div key={i} className="border-2 border-slate-200 rounded-2xl p-8 text-center hover:shadow-lg transition-shadow">
                                <div className={`w-16 h-16 ${comm.color} rounded-2xl flex items-center justify-center text-white mx-auto mb-4`}>{comm.icon}</div>
                                ...
                                <button className={`w-full py-3 ${comm.color} text-white rounded-full font-semibold hover:opacity-90 transition-opacity`}>{comm.cta}</button>
                            </div>
                        ))}
                    </div>
                </div>
                    ...
                    <div className="space-y-4">
                        {upcomingEvents.map((event, i) => (
                            <div key={i} className="bg-white p-6 rounded-xl flex flex-col md:flex-row md:items-center justify-between shadow-sm">
                                ...
                                <button className="mt-4 md:mt-0 px-6 py-2 bg-slate-900 text-white rounded-full font-semibold text-sm hover:bg-slate-800 transition-colors">Register</button>
                            </div>
                        ))}
                    </div>
                </div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                        {topContributors.map((c, i) => (
                            <div key={i} className="bg-slate-50 p-6 rounded-xl text-center">
                                ...
                                <p className="text-sm text-slate-500">{c.contributions} contributions</p>
                            </div>
                        ))}
                    </div>
                </div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                        {resources.map((res, i) => (
                            <Link key={i} href={res.link} className="bg-white p-6 rounded-xl text-center shadow-sm hover:shadow-md transition-shadow">
                                <div className="w-12 h-12 bg-purple-100 rounded-xl flex items-center justify-center text-purple-600 mx-auto mb-3">{res.icon}</div>
            ...
            <section className="py-20 bg-gradient-to-br from-slate-900 to-purple-900 text-white">
                <div className="max-w-4xl mx-auto px-6 lg:px-8 text-center">
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 justify-center">
                        <Link href="/demo" className="px-8 py-4 bg-gradient-to-r from-orange-500 to-pink-500 text-white rounded-full font-semibold text-lg shadow-xl hover:scale-105 transition-all">Start Free Trial</Link>
                        <button className="px-8 py-4 bg-indigo-500 text-white rounded-full font-semibold text-lg hover:bg-indigo-400 transition-all">Join Discord</button>
                    </div>
                </div>
            </section>
        </SiteLayout>
