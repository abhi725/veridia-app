        ...
        >
            ...
            <aside className="relative w-20 md:w-64 bg-white/90 backdrop-blur-xl border-r border-slate-200/80 flex flex-col items-center md:items-start py-8 z-50 shadow-sm">
                <div className="px-6 mb-10 w-full flex items-center gap-3">
                    <div className="w-8 h-8 bg-gradient-to-br from-swan-charcoal to-slate-700 rounded-lg shadow-md"></div>
                    <span className="hidden md:block font-bold text-xl text-swan-charcoal">SwanDigitals</span>
                </div>
                ...
                <header className="relative h-16 border-b border-slate-200/80 bg-white/70 backdrop-blur-md px-8 flex items-center justify-between z-40">
                    ...
                    <div className="flex items-center gap-4">
                        <div className="flex items-center gap-2">
                            ...
                            <button
                                onClick={onCollapse}
                                className="w-12 h-6 bg-slate-200 rounded-full relative transition-all hover:bg-swan-orange/20 cursor-pointer group"
                            >
                                <div className="absolute left-1 top-1 w-4 h-4 bg-white rounded-full shadow-sm transition-all group-hover:bg-swan-orange"></div>
                            </button>
                        </div>
                        <button className="p-2 hover:bg-slate-100 rounded-full transition-colors"><Bell size={20} className="text-slate-500" /></button>
                        <div className="w-8 h-8 bg-gradient-to-br from-orange-400 to-swan-orange rounded-full shadow-sm"></div>
                    </div>
                </header>
...
function NavItem({ icon, label, active = false }: { icon: any, label: string, active?: boolean }) {
    return (
        <button className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-all ${active ? 'bg-orange-50 text-swan-orange shadow-sm' : 'text-slate-500 hover:bg-slate-50'}`}>
            {icon}
...
function ChatBubble({ user, text, time, ai = false }: { user: string, text: string, time: string, ai?: boolean }) {
    return (
        <div className={`flex flex-col ${ai ? 'items-end' : 'items-start'}`}>
            <div className={`max-w-[80%] p-4 rounded-2xl shadow-sm ${ai ? 'bg-gradient-to-br from-swan-orange to-orange-500 text-white rounded-tr-none' : 'bg-slate-100 text-slate-700 rounded-tl-none'}`}>
